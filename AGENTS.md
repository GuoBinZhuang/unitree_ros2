...
**生成时间：** 2026-04-21 10:48:00 +08:00
**提交：** `403f4e1`
**分支：** `emergency_stop`
...
├── foxglove/        # G1 可视化资源：URDF/MJCF/meshes、layout 模板、局部 bridge 脚本
├── .github/         # Foxy/Humble CI；可直接当构建脚本参考
├── .devcontainer/   # Foxy/Humble 容器开发环境
├── setup*.sh        # 本机环境入口；Foxy/Humble/Jazzy 痕迹并存
├── run_foxglove_bridge_g1.sh  # 根层 G1 可视化桥接脚本；当前与 foxglove/ 同名脚本内容一致
...
| 调整本机网络接口 | `setup.sh`, `setup_local.sh`, `setup_default.sh` | `setup.sh` 当前默认自动探测目标 IP 路由 |
...
| 查 G1 急停链路 | `example/src/src/g1/high_level/g1_emergency_stop_node.cpp`, `example/src/src/g1/GPIO/` | 前者是 ROS2 节点，后者是独立 GPIO demo |
| 查 G1 可视化/URDF 桥接 | `foxglove/AGENTS.md`, `foxglove/g1_description/README.md`, `run_foxglove_bridge_g1.sh` | 先确认模型变体，再确认 bridge 脚本来源 |
...
| `example/src/src/g1/high_level` | G1 高层控制示例 | `example/src/src/g1/high_level` | loco、motion switch、arm action、急停节点 |
| `example/src/src/g1/lowlevel` | G1 低层控制示例 | `example/src/src/g1/lowlevel` | 含 `behavior_lib` 数据目录 |
| `example/src/src/g1/GPIO` | 独立 libgpiod 急停 demo | `example/src/src/g1/GPIO` | 不走 `colcon build` |
| `foxglove/g1_description` | G1 可视化模型资源 | `foxglove/g1_description` | 大量 URDF/MJCF 变体 + meshes；不是 ROS2 package |
...
- `example/src/CMakeLists.txt` 默认 C++14，但大量目标显式升到 `cxx_std_20`；参考相邻目标，不要只看全局默认值。
- `example/src/include/nlohmann/**` 是 vendored 第三方库；默认视为只读。
- `foxglove/` 不是构建工作区；主体是 G1 可视化资产与脚本，入口文件优先看 `g1_description/README.md` 与两个 `run_foxglove_bridge_g1.sh`。
- `.github/workflows/build-foxy.yml` 与 `build-humble.yml` 直接展示了 CI 认可的依赖、环境变量、构建顺序。
- 仓库存在 ROS2 版本混用痕迹：README 与 `setup_local.sh`/`setup_default.sh` 偏 Foxy，CI 覆盖 Foxy/Humble，`setup.sh`/`setup_myself.sh` 与部分 G1 文档又出现 Jazzy。
- 根目录 `build/ install/ log/` 主要反映根层或 `example/` 的 colcon 产物；不是 `cyclonedds_ws/` 源码边界。

## 本项目反模式
- 不要把 `build/`、`install/`、`log/` 当源码目录分析或修改。
- 不要在 `example/src/include/nlohmann/**` 中做业务修改；那是第三方 vendored 代码。
- 不要把 `foxglove/g1_description/**` 当成可通过 `colcon build` 自动生成的代码；这里主要是静态模型资产。
...
- 不要把 `example/src/src/g1/GPIO/` 的独立 CMake 规则误当成主 ROS2 package 规则；它与 `g1_emergency_stop_node` 不是同一构建链。
- 不要直接照抄两个 Foxglove bridge 脚本里的硬编码 `ROOT_DIR`；它们绑定了当前机器目录与本机安装路径。
...
- G1 急停有两条并存实现：`high_level/g1_emergency_stop_node.cpp` 是 ROS2 节点；`GPIO/` 是底层检测 demo。
- Foxglove 可视化有两处同名启动脚本：根层与 `foxglove/` 当前内容一致，都是仅启动 `foxglove_bridge`，都没有实际注入 `robot_state_publisher` / `robot_description`。
...
# G1 GPIO demo（独立 CMake，不经 colcon）
cmake -S example/src/src/g1/GPIO -B example/src/src/g1/GPIO/build
cmake --build example/src/src/g1/GPIO/build
```

## 备注
- 当前环境未安装 `clangd`；LSP codemap 不可用，目录决策主要基于结构、CMake、文档与 CI。
- 当前层级：`cyclonedds_ws/`、`example/`、`docs/`、`foxglove/`、`.github/`、`example/src/src/g1/`、`example/src/src/g1/GPIO/`。
- 若 README / setup 脚本 / 文档冲突，优先信 `.github/workflows/`，再回查对应源码目录。
- 当前 AGENTS 层级：根目录、`cyclonedds_ws/`、`example/`、`example/src/src/g1/`、`example/src/src/g1/GPIO/`、`docs/`、`foxglove/`、`.github/`；未继续下钻到 `go2/`、`b2/`、`b2w/`、`h1-2/`、`common/`、`cyclonedds_ws/src/unitree/unitree_{go,hg,api}`，避免重复。
