...
```text
example/src/src/g1/
├── high_level/     # loco / motion switch / arm action / arm sdk dds / emergency stop node
...
| 改高层急停节点 | `high_level/g1_emergency_stop_node.cpp` | 依赖 `libgpiod`；和 `GPIO/` demo 不是同一构建链 |
...
- `g1_emergency_stop_node` 属于主 ROS2 package 目标，但是否构建取决于宿主机是否找到 `libgpiod`。
...
- 不要把 `g1_emergency_stop_node` 与 `GPIO/` demo 混成一个目标看待；前者是 ROS2 节点，后者是底层独立 demo。
